...
insert into wallet(ROI_Income) values (new.ROI_received);
END$$
DELIMITER ;
...
insert into wallet(Referral_income) values (new.Referral_income);
END$$
DELIMITER ;
...
insert into wallet(Level_Income) values (new.Level_income_Rec);
END$$
DELIMITER ;

-- table 8 (reward)
create table Reward (Id int auto_increment primary key , Reward_name varchar(50), Reward varchar(50), Business_required varchar(50) , 
Status boolean);

-- table 9 (user reward )
create table User_Reward (Id int auto_increment primary key , UserId int , foreign key (UserId) references User(UserId) 
on update cascade on delete cascade, RewardId int , foreign key (RewardId) references Reward(Id) on update cascade on delete cascade,
Date datetime , Reward_rec varchar(30));

DELIMITER $$
CREATE DEFINER=`root`@`localhost` TRIGGER `user_reward_AFTER_INSERT` AFTER INSERT ON `user_reward` FOR EACH ROW BEGIN
insert into wallet(Reward_income) values (new.Reward_Rec);
END$$
DELIMITER ;

-- table 11 (ledger)
create table Ledger (Id int auto_increment primary key , Wallet_Id int , foreign key (Wallet_Id) references Wallet(Id) 
